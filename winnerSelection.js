...
async function main() {
	...
	const gainFilesPrefixes = [
		...
		"Te_matrices/Te_3690",
		"Te_matrices/Te_3696",
		"Te_matrices/Te_36102",
		"Te_matrices/Te_36108",
		"Te_matrices/Te_36114",
		"Te_matrices/Te_36120",
	];

	const financialBenefitFilesPrefixes = [
		"FUE_Bid_Files/FUE_database_3648",
		"FUE_Bid_Files/FUE_database_3654",
		"FUE_Bid_Files/FUE_database_3660",
		"FUE_Bid_Files/FUE_database_3666",
		"FUE_Bid_Files/FUE_database_3672",
		"FUE_Bid_Files/FUE_database_3678",
		"FUE_Bid_Files/FUE_database_3684",
		"FUE_Bid_Files/FUE_database_3690",
		"FUE_Bid_Files/FUE_database_3696",
		"FUE_Bid_Files/FUE_database_36102",
		"FUE_Bid_Files/FUE_database_36108",
		"FUE_Bid_Files/FUE_database_36114",
	...
	const fuePaths = generateFileNames(
		financialBenefitFilesPrefixes,
		numFilesPerPrefix,
	);
	...
	const precision = 2;
	const alpha = 1.0; // weight of bid
	const betta = (1 - alpha).toFixed(precision);

	let optimalGains = {};
	let totalGainBenefits = {};
	let totalBidBenefits = {};
	...
	for (let i = 0; i < fuePaths.length; i++) {
		...
		const calculateAverage = (data) => {
			...
			for (const key in data) {
				...
				dataFue[key - 1].qualityFactor =
					alpha * dataFue[key - 1].Bid * bidMultiplier + betta * average;
				result[key] =
					alpha * dataFue[key - 1].Bid * bidMultiplier + betta * average;
			}
			return result;
		...
		const qualitySorted = Object.entries(quality).sort(([, a], [, b]) => b - a);
		...
		let totalBidBenefit = 0;
		let totalGainBenefit = 0;
		for (const fue in resourceMap) {
			totalChannelGain +=
				parseFloat(dataFue[parseInt(fue.split(" ")[1]) - 1].Bid) * alpha +
				parseFloat(dataTe[parseInt(fue.split(" ")[1]) - 1][resourceMap[fue]]) *
					betta;
			totalBidBenefit = parseFloat(
				dataFue[parseInt(fue.split(" ")[1]) - 1].Bid,
			);
			totalGainBenefit = parseFloat(
				dataTe[parseInt(fue.split(" ")[1]) - 1][resourceMap[fue]],
			);
		}

		optimalGains[`Te_${lengthMueList}${lengthFueList}_${iter}.csv`] =
			totalChannelGain;
		totalBidBenefits[`Te_${lengthMueList}${lengthFueList}_${iter}.csv`] =
			totalBidBenefit;
		totalGainBenefits[`Te_${lengthMueList}${lengthFueList}_${iter}.csv`] =
			totalGainBenefit;

		const myData = Object.entries(resourceMap).map(([fue, MUE]) => ({
			fue,
			MUE,
		}));
		...
		try {
			...
			fs.writeFileSync(
				`./winnerSelection/output_${lengthMueList}${lengthFueList}_${iter}.csv`,
				csv,
			);
		} catch (err) {
			console.error(err);
	...
	const optimalGainsArray = Object.entries(optimalGains).map(
		([key, value]) => ({
			CSV_File: key,
			Optimal_Gain: value,
		}),
	...
	const totalBidBenefitsArray = Object.entries(totalBidBenefits).map(
		([key, value]) => ({
			CSV_File: key,
			Optimal_Gain: value,
		}),
	...
	const totalGainBenefitsArray = Object.entries(totalGainBenefits).map(
		([key, value]) => ({
			CSV_File: key,
			Optimal_Gain: value,
		}),
	...
	fs.writeFileSync(
		`WinnerSelectionCSV/alpha${alpha.toFixed(precision)}_beta${parseFloat(betta).toFixed(precision)}OptimalGains.csv`,
		csvOptimalGains,
	);

	const csvBenefitGains = parserOptimalGains.parse(optimalGainsArray);
	fs.writeFileSync(
		`WinnerSelectionCSVBenefit/alpha${alpha.toFixed(precision)}_beta${parseFloat(betta).toFixed(precision)}OptimalGains.csv`,
		csvOptimalGains,
	);

	const csvChannelGains = parserOptimalGains.parse(optimalGainsArray);
	fs.writeFileSync(
		`WinnerSelectionCSVGain/alpha${alpha.toFixed(precision)}_beta${parseFloat(betta).toFixed(precision)}OptimalGains.csv`,
		csvOptimalGains,
	);
}
